...
def prep_data_from_csv(csv_file, vocab):
    ...
    data['Tweet'] = data['Tweet'].apply(lambda x: [vocab[token] if token in vocab else 1 for token in x]) # replaced "<UNK>" with 1
...
if __name__ == "__main__":
    ...
    pad_token = 0  # Define the padding value
    inputs_tensors = [torch.LongTensor(seq) for seq in inputs]
    inputs = pad_sequence(inputs_tensors, batch_first=True, padding_value=pad_token)

    # Convert inputs and labels to tensors and create DataLoader
    inputs_tensor = pad_sequence(inputs_tensors, batch_first=True, padding_value=pad_token)
